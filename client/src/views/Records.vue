<template>
    ...
    <v-container fluid  >
        <v-card v-if="isloading== false" class="mt-3"  :class="[isloading == false ? `imagehover` : '']">
            <v-row>
        ...
        <!-- Person Involved -->
            ...
            <v-tab-item value='3'>
                <v-card class='pa-2 MyCard1'> 
                    <p  style="font-size:11px; margin-right:10px" align='right'><a href="http://hv12-hrd41:3001/ftp/FileServer/SafetyInvestigatoryDatabank/NoPicture.png" >Download NO Picture Available here</a></p>
                    <v-row class='pt-2 pb-5'>
                        <v-row>
                            <v-col  style='margin-left:100px' cols='5' align="center">
                                <v-file-input  
                                    v-model="editItems.PictureCause" 
                                    style="max-width:calc(100vh - 450px);min-width:calc(100vh - 450px);" 
                                    accept="image/*" 
                                    :clearable='false'
                                    @change="onFileChange" 
                                    ></v-file-input>
                            ...
                            <v-col   style='margin-left:40px' cols='5' align="center">
                                <v-file-input  
                                    ...
                                    style="max-width:calc(100vh - 450px);min-width:calc(100vh -0px);"  
                                    accept="image/*" 
                                    :clearable='false'
...
export default {
    ...
    data(){
        return{
            ...
            DepartmentURL:'http://adminsql1/api/company/department',
            ftpPath:'http://hv12-hrd41:3001/ftp/FileServer/SafetyInvestigatoryDatabank',
            EmployeeInfoURL:'http://adminsql1/api/basicinfo',
            SafetyImageURL:'//hv12-hrd41/D/Deployment/SafetyInvestigatoryDatabank',
            leadersURL:'http://adminsql1:8700/api/employees',
    ...
    methods:{
        ...
        EmployeeInfo(){
            ...
            if(this.infoData == undefined || this.infoData == '' || this.editItems.EmployeeID == ''){
                    this.infoData = {}
                    this.editItems.Name = ''
        ...
        async SirReportButton(item){
            ...
            this.companyheader = 'FM-HTI 02-FSE-49 (11) ;September 16,2021'
            ...
            fetch(`${this.ftpPath}/${this.CompanyPath}/${item.SirNo}/${this.PictureEffect}`, { method: 'HEAD' })
                .then(res => {
                    if (!res.ok) {
            ...
            fetch(`${this.ftpPath}/${this.CompanyPath}/${item.SirNo}/${this.PictureCause}`, { method: 'HEAD' })
                .then(res => {
                    if (!res.ok) {
                        const Toast = Swal.mixin({
                        toast: true,
